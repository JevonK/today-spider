...
def FindWindow(chatroom):
    ...
    def find_windows_by_title(title):
        ...
        for hwnd in windows:
            if win32gui.GetWindowText(hwnd) == title and win32gui.GetClassName(hwnd) == 'WeChatMainWndForPC':
                result.append(hwnd)
        
        return result
    windows = find_windows_by_title("微信")
    for x in windows:
        print(x)
        win32gui.ShowWindow(x, win32con.SW_SHOWMINIMIZED)
        win32gui.ShowWindow(x, win32con.SW_SHOWNORMAL)
        win32gui.ShowWindow(x, win32con.SW_SHOW)
        win32gui.SetWindowPos(x, win32con.HWND_TOP, 0, 0, 500, 700, win32con.SWP_SHOWWINDOW)
        win32gui.SetForegroundWindow(x)  # 获取控制
        time.sleep(1)
    exit()
...
SendWxMsg('wxid_ejw2m1qn727m12','Python发送微信消息')
